...
public class StrategyRunnable implements Runnable{
    ...
    double evaluation(Player player, MoveNode[] moves, int depth) {
        ...
        Move firstMove = movesToEvaluate.getFirst();
    ...
    private double max(int depth, double alpha, double beta, Player player, MoveNode parentMoveNode) throws InterruptedException {
        ...
        for (MoveNode currentMoveNode : currentMoveNodes) {
            localGameBoard.executeCompleteTurn(currentMoveNode, player);
            ...
            localGameBoard.reverseCompleteTurn(currentMoveNode, player);
            if (wert > maxWert) {
                maxWert = wert;
                if (maxWert >= beta) {
                    break;
